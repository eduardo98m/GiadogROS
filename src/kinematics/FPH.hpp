/*
    ...
    Foot placement heuristic function.
...
Eigen::MatrixXd FPH(
    ...
    for (int i = 0; i < 4; i++) 
    {
        ...
        position_delta(0) = 1.7 * cos(command_dir) * dt * ftg_sine_phases(i) * ftg_freqs(i) * 
                            LEG_SPAN;
        
        // 1 is for the y position
        position_delta(1) = 1.02 * sin(command_dir) * dt * ftg_sine_phases(i) * ftg_freqs(i) * 
                            LEG_SPAN;
